...
- **Camera layouts** - switch between the floating overlay, camera-only view, full-height split screen, and a screen with a smaller camera beside it. Set camera width from 10–70%, reverse sides, and adjust padding and gap in the Camera inspector. Choose Fit to preserve the full screen or Cover to fill its panel, match screen corners to the camera or adjust them separately, and keep the detected face centered as the camera width changes. Use the camera layout menu beside the timeline clock to start a layout at the playhead; earlier segments keep their settings. Each camera segment shows its layout and can be selected to adjust width, corners, or face centering, or right-clicked to change its layout. Drag either edge to resize a layout segment; shared boundaries move with the neighboring segment, and a complete drag is one undo step. Use Start New Camera Segment Here to vary settings within the same layout. Layout changes ease between segments in preview and export, Fit/Cover smoothly resizes the screen, and face tracking filters detection jitter. Camera and screen resizing share one rendered frame. The Frame shadow setting applies to both the screen and camera, with shadows following transition blur and fade. Open Layout transition in the Camera inspector to set duration (0–2 seconds), easing or spring bounce, blur, and fade for the incoming segment. Reset transition restores the smooth 0.42-second default with effects off; the selected duration is honored unless the entire incoming segment is shorter. Right-click a camera segment to Copy Transition, Paste Transition, or Copy to All Transitions; only transition settings change, and applying to all is one undo step. Save named transition presets in the sidebar and reuse them across projects. Blur has independent start and duration controls, measured in seconds, and always clears by the transition boundary. Layouts and transition settings save per camera clip and carry through to export.
