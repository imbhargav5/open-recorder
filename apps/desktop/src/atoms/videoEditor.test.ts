...
describe("videoEditor atoms – loading and error state", () => {
	...
	it("video playback runtime atoms can be reset together", () => {
		const store = createStore();
		...
		store.set(videoPlaybackAnnotationVisibilityTickAtom, 3);
...
describe("videoEditor atoms – regions and selections", () => {
	...
	it("crop control drag atoms can be reset together", () => {
		const store = createStore();
		const crop: CropRegion = { x: 0.2, y: 0.2, width: 0.5, height: 0.5 };
		store.set(cropControlDragHandleAtom, "left");
		store.set(cropControlDragStartAtom, { x: 0.3, y: 0.4 });
		store.set(cropControlInitialCropAtom, crop);
...
describe("videoEditor atoms – export state", () => {
	...
	it("exportDialogShowSuccessAtom can be toggled during export completion", () => {
		const store = createStore();
		store.set(exportDialogShowSuccessAtom, true);
		expect(store.get(exportDialogShowSuccessAtom)).toBe(true);
		store.set(exportDialogShowSuccessAtom, false);
	...
	it("add custom font dialog atoms reset editable fields together", () => {
		const store = createStore();
