...
describe("sourceSelector atoms – write / read", () => {
	...
	it("sourceSelectorTabAtom accepts all SourceSelectorTab values", () => {
		const tabs: SourceSelectorTab[] = ["screens", "windows", "area"];
		const store = createStore();
	...
	it("sourceSelectorContextAtom tracks screenshot and recording contexts", () => {
		const store = createStore();
		expect(store.get(sourceSelectorContextAtom)).toBe("recording");
		store.set(sourceSelectorContextAtom, "screenshot");
		expect(store.get(sourceSelectorContextAtom)).toBe("screenshot");
	});
	...
	it("sourcesAtom can be populated with area sources", () => {
		const store = createStore();
		const source = makeSource({
		...
		store.set(sourcesAtom, [source]);
		expect(store.get(sourcesAtom)[0].sourceType).toBe("area");
		expect(store.get(sourcesAtom)[0].captureSourceId).toBe("screen:1:0");
	});
