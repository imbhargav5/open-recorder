...
describe("recording atoms – write / read", () => {
	...
	it("recordingActiveAtom is derived from recordingPhaseAtom", () => {
		const store = createStore();
		store.set(recordingPhaseAtom, "starting");
		expect(store.get(recordingActiveAtom)).toBe(false);
		store.set(recordingPhaseAtom, "recording");
		expect(store.get(recordingActiveAtom)).toBe(true);
		store.set(recordingPhaseAtom, "interrupted");
		expect(store.get(recordingActiveAtom)).toBe(false);
	});
