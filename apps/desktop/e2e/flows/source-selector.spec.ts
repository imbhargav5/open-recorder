...
import { fakeScreenSources, fakeWindowSources } from "../fixtures/fake-sources";
import { configureHandlers, configureSourceHandlers, installTauriShim } from "../setup/tauri-shim";
...
test.describe("Source Selector", () => {
	...
	test("Area tab is visible for recording source selection", async ({ page }) => {
		await configureSourceHandlers(page, {
			screenSources: fakeScreenSources(),
			windowSources: fakeWindowSources(),
		});
		...
		await expect(page.getByRole("tab", { name: /area/i })).toBeVisible();
	});

	test("Area tab is hidden for screenshot source selection", async ({ page }) => {
		await configureSourceHandlers(page, {
			screenSources: fakeScreenSources(),
			windowSources: fakeWindowSources(),
		});
		await page.goto(`${SOURCE_SELECTOR_URL}&context=screenshot`);
		...
		await expect(page.getByRole("tab", { name: /screens/i })).toBeVisible();
		await expect(page.getByRole("tab", { name: /windows/i })).toBeVisible();
		await expect(page.getByRole("tab", { name: /area/i })).toHaveCount(0);
	});

	test("Area draw selection creates and shares an area source", async ({ page }) => {
		await configureSourceHandlers(page, {
			screenSources: fakeScreenSources(),
			windowSources: fakeWindowSources(),
		});
		...
		await expect
			.poll(() => page.evaluate(() => window.__IPC_GET_CALLS__("select_source").length))
			.toBe(1);

		const calls = await page.evaluate(() => window.__IPC_GET_CALLS__("select_source"));
		expect(calls[0].args.source).toMatchObject({
			id: "area:0:24:32:1280:720",
			name: "Area 1280x720",
			sourceType: "area",
			captureSourceId: "screen:0:0",
			areaSelection: {
	...
	test("canceling area draw leaves the selected source unchanged", async ({ page }) => {
		await configureSourceHandlers(page, {
			screenSources: fakeScreenSources(),
			windowSources: [],
		});
		...
		await expect(page.getByText("Draw a recording area")).toBeVisible();
	});

	test("screen sources appear as buttons after getSources resolves", async ({ page }) => {
	...
	test("empty state message shows when no sources are available", async ({ page }) => {
		await configureHandlers(page, { get_sources: [] });
		await page.goto(SOURCE_SELECTOR_URL);
	...
	test("getSources IPC call is logged on mount", async ({ page }) => {
		...
		const wasCalled = await page.evaluate(() => window.__IPC_WAS_CALLED__("get_sources"));
